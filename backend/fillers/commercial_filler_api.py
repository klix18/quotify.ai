...
async def render_commercial_pdf(data: dict) -> bytes:
    """Render the commercial HTML template with data and return PDF bytes."""
...
async def generate_commercial_quote(
    payload: dict,
    user: dict = Depends(get_current_user),
):
    try:
        pdf_bytes = await render_commercial_pdf(payload)
